...
public class GlobalExceptionHandler {
    ...
    public ModelAndView shiroPermissionsErrorHandler(javax.servlet.http.HttpServletRequest request, HttpServletResponse response, Object object, UnauthorizedException exception) {
        // 判断是否ajax请求
    ...
    public ModelAndView defaultErrorHandler(javax.servlet.http.HttpServletRequest request, HttpServletResponse response, Object object, Exception exception) {
        ...
        if (!(request.getHeader("accept").indexOf("application/json") > -1 || (request
            ...
            exception.printStackTrace();
            //对于非ajax请求，我们都统一跳转到error页面
            return new ModelAndView("/rnt/sys/readypage", map);
        } else {
            // 如果是ajax请求，JSON格式返回
            try {
                response.setContentType("application/json;charset=UTF-8");
