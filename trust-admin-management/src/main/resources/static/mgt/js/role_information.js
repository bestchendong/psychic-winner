...
function rollAddFormSubmit(){
    if(addFormValidate()){
        $.post("/mgt/role/add",$("#addForm").serialize(),function(data){
            if(data.code == 200){
...
function editModal(id) {
    //可以采用传参的方式而不向后台请求
    $.get("/role/detail",{roleId:id},function(role){
...
function editFormSubmit(){
    if(editFormValidate()){
        $.post("/mgt/role/edit",$("#editForm").serialize(),function(data){
            if(data.code == 200){
                window.location.reload();
            }
        });
...
function roleMenuModal(roleId){
    ...
    $.get("/mgt/role/tree/menu",{"systemId":1,"roleId":roleId},function(tree){
        if(tree != null){
            var firstMenu = "";
...
function roleOperationModal(roleId){
    $.get("/mgt/role/tree/operation",{"roleId":roleId},function(tree){
        var roleIdInput = '<input name="roleId" type="hidden" value="'+roleId+'"/>';
        var firstLevel = "";
                ...
                if(operations.length > 0) {
                    ...
                    for(var j = 0; j < operations.length; j++){
                        secondLevel +=  '<div class="button-two-content">' +
                                                '<input type="checkbox" value="'+operations[j].id+'" name="operationId" id="'+'o_'+letters[i]+'" '+((operations[j].checked)?'checked="true"':'')+'/>' +
                                                '<label for="'+'o_'+letters[i]+'">'+operations[j].text+'</label>' +
                                            '</div>' ;
                    }
...
function roleMenuFormSubmit(){
    ...
    $.post("/mgt/role/set/menu",{"roleId":roleId,"menuIds":menuIds},function(data){
        if(data.code == 200){
            $("#myModal2").modal('hide');
            //window.location.reload();
        }
...
function roleOperationFormSubmit(){
    ...
    $.post("/mgt/role/set/operation",{"roleId":roleId,"operationIds":operationIds},function(data){
        if(data.code == 200){
            $("#myModal3").modal('hide');
            //window.location.reload();
        }
...
function deleteRole(roleId){
    ...
    if(x == true){
        console.log('delete');
        $.post("/mgt/role/delete",{"roleId":roleId},function(data){
            if(data.code == 200){
                window.location.reload();
            }
        })
