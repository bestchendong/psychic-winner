...
public class RoleServiceImpl implements RoleService {
    ...
    @Override
    public List<Menu> getMenusOfRole(int roleId) {
    ...
    public List<Tree> getOperationTreeWithRole(int roleId) {
        ...
        if(menuIds != null && menuIds.size() > 0){
            MenuExample menuExample = new MenuExample();
            menuExample.createCriteria().andMenuIdIn(menuIds).andLevelEqualTo(4);
            List<Menu> menuList = menuMapper.selectByExample(menuExample);
    ...
    public List<Role> getActiveRoleList() {
        RoleExample roleExample = new RoleExample();
        roleExample.createCriteria().andStatusEqualTo(1);
        return roleMapper.selectByExample(roleExample);
    }
